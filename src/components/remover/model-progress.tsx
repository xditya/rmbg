 ...
 * total that is revised upwards must not pull the bar back.
 */
export function ModelProgress({ loaded = 0, total = 0, mode }: { loaded?: number; total?: number; mode: "determinate" | "indeterminate" }) {
  ...
  const [shown, setShown] = useState({ mode, fraction });
  if (shown.mode !== mode || fraction > shown.fraction) setShown({ mode, fraction });
  const shownFraction = shown.mode === mode ? Math.max(shown.fraction, fraction) : fraction;
  return (
    <div
      role="progressbar"
