...
export function Notice({ card, onRetry, onRemove, className }: { card: Card; onRetry: () => void; onRemove: () => void; className?: string }) {
  ...
  return (
    <div role="alert" className={cn("flex items-start gap-2.5 rounded-md border border-border bg-surface px-3 py-2.5 text-[13px] animate-fade-up max-sm:flex-wrap", className)}>
